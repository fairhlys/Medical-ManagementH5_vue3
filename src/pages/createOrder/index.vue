<script setup>
import { useRouter } from 'vue-router'
import { ref, onMounted, computed } from "vue";
import statusBar from '@/components/statusBar.vue'
import { createGetOrder, createOrder } from '@/api/createOrder'
import { lock } from '@/utils/lock';
import { showNotify } from 'vant';
import 'vant/es/notify/style';
import QRCode from 'qrcode'
const router = useRouter()

const goback = () => {
    router.go(-1)
}
const showHospital = ref(false)
const createInfo = ref({})
onMounted(async () => {
    const res = await createGetOrder()
    console.log('订单数据', res.data);
    createInfo.value = res.data;
})
const showHoscolumns = computed(() => {
    return createInfo.value.hospitals.map(item => {
        return { text:item.name, value: item.id }
    })
})
const onConfirm = (item) => {
    form.value.hospital_id = item.selectedOptions[0].value
    form.value.hospital_name = item.selectedOptions[0].text
    showHospital.value = false
}
const form = ref({})
const showTime = ref(false)
const currentDate = ref()
const showDate = ref()
const minDate = ref(new Date())
const onTimeConfirm = (date) => {
    const datestr = date.selectedValues.join('-')
    currentDate.value = date.selectedValues 
    showDate.value = datestr
    form.value.starttime = new Date(datestr).getTime() 
    showTime.value = false
}
const showComponion = ref(false)
const onComponionConfirm = (item) => {
    form.value.companion_id = item.selectedOptions[0].value
    form.value.companion_name = item.selectedOptions[0].text
    showComponion.value = false
}
const showComcolumns = computed(() => {
    return createInfo.value.companion.map(item => {
        return { text:item.name, value: item.id }
    })
})
const showCode = ref(false)
const CodeImg = ref('')
const lockSubmit = lock(createOrder)
const submitOrder = async () => {
    try {
        for(const  i of ['hospital_id', 'starttime', 'companion_id', 'receiveAddress', 'tel', 'demand']) {
            if(!form.value[i]) {
                showNotify({ type: 'danger', message: '请完善订单信息' });
                throw new Error('请完善订单信息')
            }
        }
        console.log('提交订单', form.value);
        const res = await lockSubmit(form.value);
        console.log(res);
        showNotify({ type: 'success', message: '订单提交成功' });
        //将订单提交后，返回的url地址转变为二维码，展示给用户扫码支付
        //使用插件pnpm add qrcode@latest
        QRCode.toDataURL(res.data.wx_code).then(url => {
            showCode.value = true
            CodeImg.value = url
            console.log('生成的二维码地址', url);
            //这里可以将url展示在页面上，或者使用其他方式展示二维码
        }).catch(err => {
            console.error('生成二维码失败', err);
        });
    } catch (error) {
        showNotify({ type: 'danger', message: error.message || '订单提交失败' });
        console.log('表单验证失败', error);
        return
    }
}
const closeCode = () => {
    showCode.value = false
    router.push('/order')
}
</script>
<template>
    <div class="container">
        <div class="header">
            <van-icon @click="goback" name="arrow-left" class="header-left" size="30" />
            填写服务订单
        </div>
        <statusBar item="0"></statusBar>
        <van-cell class="cell" center>
            <template #title>
                <van-image width="25" height="25" :src="createInfo.service?.serviceImg" />
                <span class="server-name">{{ createInfo.service?.serviceName }}</span>
            </template>
            <template #default>
                <div class="service-text">服务内容</div>
            </template>
        </van-cell>
        <van-cell-group class="cell">
            <van-cell class="cell" center>
                <template #title>
                    就诊医院
                </template>
                <template #default>
                    <div @click="showHospital =  true">{{ form.hospital_name || '请选择就诊医院' }}<van-icon name="arrow" /></div>
                    
                </template>
            </van-cell>
            <van-cell class="cell" center>
                <template #title>
                    就诊时间
                </template>
                <template #default>
                    <div @click="showTime =  true">{{ showDate || '请选择就诊时间' }}<van-icon name="arrow" /></div>
                </template>
            </van-cell>
            <van-cell class="cell" center>
                <template #title>
                    陪诊师
                </template>
                <template #default>
                    <div @click="showComponion =  true">{{ form.companion_name || '请选择陪诊师' }}<van-icon name="arrow" /></div>
                </template>
            </van-cell>
            <van-cell class="cell" center>
                <template #title>
                    接送地址
                </template>
                <template #default>
                    <van-field class="text" input-align="right" v-model="form.receiveAddress" placeholder="请输入接送地址" :rules="[{required:true,message:'请输入接送地址'}]" />
                </template>
            </van-cell>
            <van-cell class="cell" center>
                <template #title>
                    联系电话
                </template>
                <template #default>
                    <van-field class="text" input-align="right" v-model="form.tel" placeholder="请输入联系电话" :rules="[{required:true,message:'请输入联系电话'}]" />
                </template>
            </van-cell>
        </van-cell-group>
        <van-cell-group title="服务需求" class="cell">
            <van-field class="text" style="height: 100px;" v-model="form.demand" placeholder="请简单描述您要就诊的科室" :rules="[{required:true,message:'请简单描述您要就诊的科室'}]" />
        </van-cell-group>
        <van-button class="sumbit" size="large" round type="primary" @click="submitOrder">提交订单</van-button>
        <van-popup
        v-model:show="showHospital"
        position="bottom"
        :style="{ height: '30%' }"
        >
            <van-picker
            :columns="showHoscolumns"
            @confirm="onConfirm"
            @cancel="showHospital = false"
            />
        </van-popup>
        <van-popup
        v-model:show="showTime"
        position="bottom"
        :style="{ height: '30%' }"
        >
           <van-date-picker
            v-model="currentDate"
            title="选择日期"
            :min-date="minDate"
            @confirm="onTimeConfirm"
            @cancel="showTime = false"
            />
        </van-popup>
        <van-popup
        v-model:show="showComponion"
        position="bottom"
        :style="{ height: '30%' }"
        >
            <van-picker
            :columns="showComcolumns"
            @confirm="onComponionConfirm"
            @cancel="showComponion = false"
            />
        </van-popup>
        <!-- 支付二维码弹窗 -->
        <van-dialog v-model:show="showCode" :show-confirm-button="false">
            <van-icon name="cross" class="close" @click="closeCode" />
            <div>微信支付</div>
            <van-image :src="CodeImg" width="150" height="150" />
            <div>请使用微信扫描二维码完成支付</div>
        </van-dialog>
    </div>
</template>
<style lang="less" scoped>
.container {
  background-color: #f0f0f0;
  height: 100vh;
}
.header {
  padding: 10px 0;
  text-align: center;
  line-height: 30px;
  font-size: 15px;
  .header-left {
    float: left;
  }
}

.cell {
  width: 95%;
  margin: 5px auto;
  background-color: #fff;
  ::v-deep(.van-field__control) {
    color: var(--van-cell-value-color);
  }
  ::v-deep(.van-cell__title) {
	display: flex;
	align-items: center;
  }
  .server-name {
	  margin-left: 10px;
  }
}
.service-text {
  background: url(data:image/png;base64,)
    no-repeat center center;
  background-size: 20px;
}
.sumbit {
  position: absolute;
  bottom: 0;
}
::v-deep(.van-dialog__content) {
  text-align: center;
  padding: 20px;
  .close {
    position: absolute;
    left: 20px;
  }
}</style>